...
bool cuavantAcceptat(productie *gramaticaRegulata, int numarProductii, int neterminala)
{
    int i;
	...
	if (contorCuvant < lungimeMaxima)
    {
		for (i = 0; i < numarProductii; i++)
			if (gramaticaRegulata[i].neterminala1 == neterminala && gramaticaRegulata[i].terminala != "?")
            {
				...
				generareCuvinte(gramaticaRegulata, numarProductii, lungimeMaxima, cuvant, contorCuvant + 1, gramaticaRegulata[i].neterminala2);
            }
    }
}
